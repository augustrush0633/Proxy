...
[rewrite_local]
^https:\/\/whos\.tv\/api\/(login|user\/profile|user\/statistics|user\/tasks\/today-points|user\/tasks\/signin) url script-request-header https://raw.githubusercontent.com/augustrush0633/Proxy/refs/heads/main/Loon/Plugins/WhosTVSignin.js
...
var CAPTURE_TIME_KEY = "WHOSTV_LAST_CAPTURE_TIME";
var CAPTURE_INTERVAL_MS = 2000; // 抓取节流间隔：2 秒
...
function captureCookie() {
  ...
  // ---- 节流控制：距离上次抓取不足 2 秒则跳过 ----
  var now = Date.now();
  var lastTimeRaw = readStore(CAPTURE_TIME_KEY);
  var lastTime = parseInt(lastTimeRaw, 10);

  if (!isNaN(lastTime) && (now - lastTime) < CAPTURE_INTERVAL_MS) {
    console.log("[" + SCRIPT_NAME + "] 抓取过于频繁，已跳过（距上次 " + (now - lastTime) + "ms）");
    return;
  }

  // 无论后续是否成功，先更新时间戳，防止短时间内重复触发
  writeStore(CAPTURE_TIME_KEY, String(now));
  ...
  var oldCookie = readStore(COOKIE_KEY);
  if (oldCookie === cookie) {
    console.log("[" + SCRIPT_NAME + "] Cookie 未发生变化，跳过保存与通知");
    return;
  }
...
function getHeader(headers, name) {
  ...
  for (var k in headers) {
    if (Object.prototype.hasOwnProperty.call(headers, k) && k.toLowerCase() === lowerName) {
      return headers[k];
    }
  }
  return "";
